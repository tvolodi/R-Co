...
// raises C40P01 (deadlock detected).
//
// Pre-fix, one of the two calls would race against the audit-trigger lock
// keyspace during migration 1107 (DROP TRIGGER IF EXISTS ... ON
// audit_entries) and the per-tenant chain-hash advisory lock; this
// surfaced as a C40P01 deadlock in TC-TNT-01-01 / TC-TNT-03-01.
// ---------------------------------------------------------------------------
test "ISS-0129 RT-1: concurrent provisionTenantSchema for two tenants serializes without deadlock" {
    ...
    // Cleanup is unconditional: even on a mid-test failure both schemas must
    // be dropped so subsequent test runs are not polluted.
    defer cleanupTenant(alloc, &pool_a, tenant_a, schema_a);
    defer cleanupTenant(alloc, &pool_b, tenant_b, schema_b);

    // Spawn two threads that each call provisionTenantSchema concurrently
    // for a distinct tenant. With the fix, the migrate-step advisory lock
    // serializes them so neither can raise C40P01; without the fix, one of
    // the two would deadlock at migration 1107.
    var t1 = try std.Thread.spawn(.{}, provisionTenantSchema, .{
        alloc, &pool_a, tenant_a, migrationsDir(),
    });
    var t2 = try std.Thread.spawn(.{}, provisionTenantSchema, .{
        alloc, &pool_b, tenant_b, migrationsDir(),
    });
    t1.join();
    t2.join();

    // RT-5 source-stability assertion runs inline as part of RT-1's preamble
    // — see end-of-file for the standalone RT-5 test as well. Reading the
    // file as text here catches a prefix drift the moment RT-1 starts
    // exhibiting deadlock symptoms again, with a clear failure message.
    const src_path = "src/db/migrations.zig";
    const content = std.Io.Dir.cwd().readFileAlloc(std.testing.io, src_path, alloc, std.Io.Limit.limited(1 << 20)) catch |err| {
        std.debug.print("RT-5 inline: could not read {s}: {}\n", .{ src_path, err });
        return err;
    };
    defer alloc.free(content);
    const needle = "hashtext('bpm.migrations.runForSchema')";
    if (std.mem.indexOf(u8, content, needle) == null) {
        std.debug.print("RT-5 inline: lock prefix drift — {s} no longer contains {s}\n", .{ src_path, needle });
        return error.MissingLockPrefix;
    }

    // After both threads complete, idempotent re-run must succeed for both
    // tenants (the fast-path branch returns immediately when
    // migrations_applied_at IS NOT NULL).
    try provisionTenantSchema(alloc, &pool_a, tenant_a, migrationsDir());
    try provisionTenantSchema(alloc, &pool_b, tenant_b, migrationsDir());

    // Confirm both tenants ended up with the same set of applied migrations
    // in public.schema_migrations — the canonical migrator did not skip any
    // files due to the lock interaction.
...
// ---------------------------------------------------------------------------
// RT-2
// Idempotent re-run after concurrent provisioning succeeds: re-running
// provisionTenantSchema for both tenants returns without error and leaves
// the migration ledger unchanged. This mirrors TC-TNT-01-01's contract
// that every provisioning attempt converges to a fully-migrated schema,
// even after the lock interaction.
// ---------------------------------------------------------------------------
test "ISS-0129 RT-2: idempotent re-run converges after concurrent provisioning" {
    const alloc = std.testing.allocator;
    ...
    var t1 = try std.Thread.spawn(.{}, provisionTenantSchema, .{
        alloc, &pool, tenant_a, migrationsDir(),
    });
    var t2 = try std.Thread.spawn(.{}, provisionTenantSchema, .{
        alloc, &pool, tenant_b, migrationsDir(),
    });
    t1.join();
    t2.join();

    // Snapshot the migration-ledger row counts after the concurrent pass.
    ...
    // Second pass: idempotent re-run. The fast-path branch (migrations_applied_at
    // IS NOT NULL) returns immediately; this asserts no error path fires and
    // the ledger row counts are unchanged (no duplicate INSERTs).
    ...
    // Both tenants must have at least one applied migration (otherwise the
    // provisioning itself failed silently, which RT-1 would have caught).
    try std.testing.expect(post_n_a > 0);
    try std.testing.expect(post_n_b > 0);
}
