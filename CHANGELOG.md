...
**ISS-0128 — `TC-DB-03-01` `InstanceNotFound` was a test-fixture bug, not the suspected `event_store` search_path race** ([GitHub #418](https://github.com/tvolodi/R-Co/issues/418))

- **The filed hypothesis (an ISS-0130-class search_path/schema-visibility race in `event_store.lookup`) does not hold.** Live instrumentation of both `src/db/pool.zig::applyRequestStorageRouting` and `src/event_store/store.zig::Store.append`'s instance lookup confirmed the connection's `search_path` was correctly `tenant_default, public` at the exact moment `StoreError.InstanceNotFound` was raised. `schemaNameForTenant` unconditionally maps the all-zero default tenant id to `tenant_default` regardless of any `storage_mode` row content, and `TestHarness.init()` already clears thread-local tenant context on entry (the ISS-0114 fix) — there was no provisioning-visibility window here to race against.
- **The actual defect: `tests/integration/db_integration_test.zig`'s `TC-DB-03-01` called `store.append()` with an instance UUID that was never inserted anywhere.** The test generates a fresh per-run UUID (`inst_id_str`) and correctly uses it for the fixture `INSERT`, the post-append assertions, and cleanup — but the `store.append()` call itself passed a separate, unrelated hardcoded literal (`"db0301aa..."`). `Store.append` correctly returned `InstanceNotFound` for a row that was genuinely never provisioned under that id; the event store's behaviour was correct throughout.
- **Fix:** added a local `parseUuid()` helper (mirroring the established pattern already used in `event_store_integration_test.zig`) and derived the `store.append()` instance id from the same `inst_id_str` used everywhere else in the test. `src/event_store/store.zig` and `src/db/pool.zig` are unchanged — confirmed defect-free by diagnosis, and `git diff --stat` on both shows zero lines changed.
- **Verification:** `zig build` exit 0, no error-set output; `tools/lint_sql_param_types.py` exit 0 (0 BLOCKER/MAJOR/MINOR). `TC-DB-03-01` reproduced failing 2/2 (100%) before the fix — once against the shared, drift-carrying `db_test` container and once against a freshly created and freshly migrated database (108/108 migrations, ruling out shared-DB drift per the ISS-0150/ISS-0151 measurement guidance) — then passed 5/5 consecutive `zig build test-integration` runs after the fix (2 fresh-DB, 3 shared-DB). Regression check by failing-test-name-set diff (not raw counts) between the pre-fix baseline and all 5 post-fix runs: the only fluctuating tests (`TC-DB-01-02`, `TC-EXP-103-03/04`, `TC-SCH-303-03`) appeared and disappeared across the 5 post-fix runs themselves with no code change between them — the pre-existing cross-binary-concurrency flakiness class already documented under ISS-0141/ISS-0148/ISS-0151, not a regression from this fix. `TC-DB-03-02`'s pre-existing exit-code-3 crash (`INVALID_HANDLE` on connection close) is present identically before and after this fix and is out of scope.
- **Prevention:** when a test introduces a per-test-random fixture id (`newUuidString()`) to fix isolation, grep the whole test body for every other identifier used in the same flow — a partial migration that leaves one call site on the old hardcoded literal reads as "isolated" until that one call site is exercised. Superseded prevention note on ISS-0128 (`event_store.lookup must qualify its tenant schema explicitly`) does not apply to this root cause; retained on the issue record for historical accuracy but not acted on, since instrumentation disproved the hypothesis it addresses. Diagnosis with full evidence: `docs/issue-reports/ISS-0128-diagnosis.yaml`.
- **No requirement status changed.** `DB-03`'s transactional-integrity guarantee already existed and was already correctly implemented; this run fixed a test that was incorrectly exercising it, not the guarantee itself.
