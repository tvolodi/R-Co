...
- A pre-existing, out-of-scope test-isolation gap in the `test-integration` umbrella step (parallel execution against one shared `bpm_test` database causes ~120 unrelated runtime failures that do not reproduce when run individually) was flagged as `ISS-503-INFRA-01` but never registered as a `docs/issues/` entry or filed on GitHub — that gap was caught during the 2026-07-30 ISS-0076 triage and formally filed as [GitHub #339](https://github.com/tvolodi/R-Co/issues/339) (ISS-0079).
