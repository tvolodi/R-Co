...
- **The gap this closes:** `instruction_limiter.zig`, `memory_limiter.zig`, and `timeout.zig` each compiled and were each correct in isolation, but none was reachable from `executor.zig` — `createSandboxedState` built every Lua state with the unbounded `defaultAlloc` and installed no hook, so a runaway script could loop forever, allocate without bound, and never observe a timeout. Worse, the limiter's own storage mechanism was forgeable: `installHook` wrote the limiter pointer into the sandboxed script's own global table (`_G.__limiter__`) via `lua_setglobal`, so any script could read, nil out, or overwrite it with a bogus light-userdata value before entering its runaway loop, defeating LUA-08 outright and risking a type-confused read of arbitrary memory if `hookCallback` trusted a forged pointer.
- **Fix:** replaced the script-writable `__limiter__` global with registry-based storage under `LUA_REGISTRYINDEX`, reusing `host_context.zig`'s already-established anti-forgery pattern (private string key, `lua_setfield`/`lua_getfield`, never `_G` — the registry is unreachable from sandboxed script code). LUA-08 and LUA-10 now share one combined `lua_sethook` callback that checks both instruction count and elapsed wall-clock time every 100 instructions, because `lua_sethook` accepts only one callback per state. LUA-10's other two acceptance criteria — a script blocked inside a host function call, and enforcement "from outside the Lua state" — needed a genuinely separate mechanism (a count-hook cannot fire while control is inside a host call), so a new host-external watchdog thread (`timeout.zig`'s `WatchdogState`/`WatchdogHandle`) was added and is now observed by `call_service.zig`'s pre/post checks. `MemoryLimiter.alloc` is wired into `lua_newstate` via `createSandboxedState` for LUA-09. `executeScript`'s unmanifested path now runs under conservative default limits (`UNMANIFESTED_DEFAULT_LIMITS`) instead of unlimited; `executeScriptWithManifest` sources its limits from the already-validated manifest fields.
- **Incidental fix (Unblock-Everything):** `service_interceptor.zig`'s `MockResponse.clone` always cloned using the `ServiceMockCatalog`'s own allocator, silently discarding the allocator passed into `executeMockedServiceCall`. This crashed the first time any test exercised the simulation path end-to-end with a real activated `simulation_runtime` and freed the response with a mismatched allocator — it directly blocked one of this run's new watchdog tests (TC-LUA-10-02) from running at all, so it was fixed in-scope rather than filed and deferred.
- **Design note honestly retained:** one sub-case — true pre-emption of an in-flight blocking syscall from outside the process — remains open; the watchdog can detect and flag it before/after the call but cannot forcibly abort it mid-flight. See `src/design/iss0169-lua08-09-10-limiter-wiring.md` §11 for the documented recommendation.
- **Verification:** five new mutation-checkable test specs (`src/lua/limiter_wiring_test.zig`, TC-LUA-08/09/10 series) wired into `src/lua_test_root.zig`. Mutation-tested by hand — reverting `installLimiter`'s `lua_sethook` call made TC-LUA-08-01/TC-LUA-10-01 hang as predicted; disabling `watchdogLoop`'s deadline check made TC-LUA-10-02/TC-LUA-10-03 fail as predicted; both reverted back afterward. Full suite PASS: 270/270 test-bearing files wired, 81/81 Lua tests (including the 5 new ones), 10/10 simulation tests. Independently re-verified by ORCH, including a hands-on mutation test of its own. No BLOCKER findings; RELEASE-VALIDATOR skipped per the WF-03 pipeline table's "BLOCKER severity only" condition.
- **Scope note — GH-495 originally covered 9 requirements (LUA-08 through LUA-16); this run resolves only LUA-08/09/10.** The remaining six were split, with explicit human-operator sign-off, into [GH-591](https://github.com/tvolodi/R-Co/issues/591) (LUA-11 variable read/write, LUA-13 logging — both need a new instance-state model, not just tests) and [GH-592](https://github.com/tvolodi/R-Co/issues/592) (LUA-12 service call, LUA-15 structured failure, LUA-16 runtime error capture — LUA-12 also gained a new deadline-awareness acceptance criterion for its future HTTP client, added during this run's LUA-10 rework, so it stays deadline-bounded against the same watchdog budget). See `docs/issues/ISS-0624.json` and `docs/issues/ISS-0625.json`.
- **Requirement status:** `LUA-08`, `LUA-09`, `LUA-10` set to `RELEASED` (previously `IN_PROGRESS` since ISS-0153/GH#471 downgraded them for having no linked interpreter).
