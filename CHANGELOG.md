...
**ISS-0143** ([GitHub #448](https://github.com/tvolodi/R-Co/issues/448) — CI `zig build test` retry loop could report PASS despite a `failed command:` line)
- **Closes GitHub #448**: `.github/workflows/ci.yml`'s bounded-retry loop for `zig build test` (added for [#436](https://github.com/tvolodi/R-Co/issues/436) / ISS-0136) treated `$status -eq 0` as sufficient proof of a pass. Observed on PR #447's CI run: attempt 2 printed the same `failed command: .../test --cache-dir=...` build-graph failure line as the timed-out attempt 1, but `zig build`'s own top-level exit code came back 0 under the same `-j2` contention #436 documents — so the loop declared PASS and moved on despite the visible failure in its own output.
- **Fix**: each attempt's combined stdout/stderr is now captured and scanned for `failed command:` (the marker Zig's build runner always prints when a child `Run` step fails) as an independent signal — `status == 0` is necessary but no longer sufficient for PASS.
- **Verification**: local bash simulation of all 3 relevant scenarios (false-positive attempt now correctly caught, genuine pass still passes, genuine all-3-failures still fails); `.github/workflows/ci.yml` YAML re-validated with `yaml.safe_load`.
- **Note**: PR #447 (ISS-0139/ISS-0140) itself was independently re-verified locally and is not believed to be affected by this gate defect — this fix is forward-looking, for future PRs that rely solely on the CI check.
