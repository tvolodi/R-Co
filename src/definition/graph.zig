...
pub fn validateGraph(
    ...
    var has_incoming = std.mem.zeroes([MAX_NODES]bool);
    var has_outgoing = std.mem.zeroes([MAX_NODES]bool);
    for (edges) |edge| {
    ...
    var adj_from = std.mem.zeroes([MAX_EDGES]u16);
    var adj_to = std.mem.zeroes([MAX_EDGES]u16);
    ...
    var is_gateway = std.mem.zeroes([MAX_NODES]bool);
    for (nodes, 0..) |node, i| {
    ...
    var on_stack = std.mem.zeroes([MAX_NODES]bool);
...
pub fn validateEdgeConditions(
    ...
    var gw_counts = std.mem.zeroes([MAX_NODES]usize);
    var gw_len: usize = 0;
