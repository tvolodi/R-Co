...
pub const Store = struct {
            ...
            if (self.service_scope_validator) |v| {
                if (tenant_id) |tid| {
                    ...
                    v.validateServiceTaskReferences(graph_to_validate, tid) catch |err| switch (err) {
                        svc_scope_validator.ServiceScopeError.ServiceNotRegistered, svc_scope_validator.ServiceScopeError.ServiceNotAvailableToTenant, svc_scope_validator.ServiceScopeError.PluginNotAvailableToTenant => {
    ...
    /// process_definitions is a PER_TENANT table reached via SCHEMA-mode
    /// search_path routing — no tenant predicate needed (ISS-0688 / GH-745).
