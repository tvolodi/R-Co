...
- `std.Io.Event` (new — synchronisation primitive introduced by this design; see `lib/std/Io.zig` ~L1766-1870 in the vendored zig 0.16.0 toolchain, `isSet` / `wait(io)` / `waitUncancelable(io)` / `waitTimeout(io, timeout)` / `set(io)` / `reset()`)
- `std.testing.io` — the `Io` implementation already available to both the server thread and the test thread inside this test binary; required as the `io` argument to `Event.wait` / `Event.set`
- `std.Io.net` / the `listen()` mechanism already used in `run()` (line 46-47) and in `src/main.zig` line 183-184
...
- Add a `std.Io.Event` field that the background thread sets once it has finished writing the captured-header fields for the request being tested, and that the test thread waits on (via `std.testing.io`) before reading those fields.
- Replace the `defer server.join()`-before-assertions ordering with an explicit event wait that happens *before* the four `expectEqual`/`expectEqualStrings` calls, not after.
...
2. **`CaptureServer` gains a new field to publish the actual bound port back to the caller**, e.g. `bound_port: u16 = 0`, plus (per point (b) below) an `Event` that also signals "bind is complete and `bound_port` is safe to read." The server does not know its real port until the OS assigns it inside `listen()`, so the existing `start()`/`run()` split needs a second, earlier synchronisation point in addition to the per-request one — see the combined event design in section (b).

3. **Inside `run()`, immediately after the `listen()` call succeeds**, read the actual bound address back from the returned server value (the same `std.Io.net.Server` value returned by `listen_address.listen(io, .{ .reuse_address = true })`, call it `listener`). The vendored `std.Io.net.Server` type (`lib/std/Io/net.zig`) is a struct of exactly two fields, `socket: Socket` and `options: AcceptOptions` — there is no `.listen_address` field and no `getLocalAddress()` accessor. The bound address, including the OS-assigned ephemeral port once `listen()` has returned, lives at `listener.socket.address` (an `IpAddress`; the `Socket.address` field's doc comment states it "contains the resolved ephemeral port number if requested"). Extract the port number from `listener.socket.address` and store it into `self.bound_port` *before* signalling readiness.
...
5. **The test builds `url_template` dynamically, after the bind-ready wait**, by formatting `"http://127.0.0.1:{}/execute"` with `server.bound_port` (using `std.fmt.allocPrint` or equivalent, freed via `defer allocator.free(...)` — this test already uses `std.testing.allocator` and already frees other allocated fields such as `cfg.node_id` in the earlier `EXT-01` parse tests in the same file; `cfg.url_template` is *not* currently freed since it's a literal today, but once it becomes an owned allocation the test must free it the same way).
...
Introduce a `std.Io.Event` field on `CaptureServer` dedicated to signalling "the current request's headers have been fully captured and it is safe to read the `first_*` fields":

1. **New field:** `headers_captured: std.Io.Event = .unset`. `Event` is a plain three-state enum (`unset` / `waiting` / `is_set`) rather than a struct with its own zero-value initialiser, so the field default is written as the explicit `.unset` variant, not `.{}`. (If the bind-readiness signal from section (a) is implemented as a second, separate `Event`, name it distinctly, e.g. `listening: std.Io.Event = .unset` — two single-shot events, one per readiness milestone, rather than overloading one event for two different meanings. A single combined event is not correct here because the test needs to observe two *different* points in the server's lifecycle: "bound and listening" (to read `bound_port`) and "first request captured" (to read `first_trace_id` etc.), and these do not happen at the same time.)

2. **Where the server sets it:** inside `run()`, in `captureRequestHeaders`'s caller, immediately after the call to `captureRequestHeaders(self, &request)` returns (line 66) and *before* the response is sent via `request.respond(...)` (lines 82-93). Concretely: right after line 66, call `self.headers_captured.set(std.testing.io)`. `Event.set` takes the `Io` instance as a parameter (it is not a bare method the way `std.Thread.ResetEvent.set()` is) — the server thread already has `std.testing.io` in scope, since `run()` already passes it to `listen()`, `server.accept()`, and the stream reader/writer. Setting the event before `respond()` (rather than after) is deliberately conservative — it guarantees the fields are fully written before any signal fires, and does not depend on the response having been transmitted. It also means the design is correct regardless of how `executeHttpRequest`'s own timing behaves, removing the implicit "the HTTP round-trip usually protects us" reasoning entirely. Per `Event.set`'s documented semantics, any memory access prior to the `set` call is released, so the waiting thread is guaranteed to observe the `first_*` field writes once its `wait` returns.
...
3. **Where the test waits on it:** in the test body, after `const result = try st.executeHttpRequest(...)` (line 335) and *before* the first assertion that reads a captured field (line 339). Call `try server.headers_captured.wait(std.testing.io)`. `Event.wait` takes the `Io` instance (the test thread also already has `std.testing.io` available, being a `std.testing`-driven test binary) and returns `Cancelable!void` — unlike `std.Thread.ResetEvent.wait()`, which is infallible, this call is fallible and must be `try`'d (or handled with `waitUncancelable(io)`, which drops the cancelation point and returns `void`, if the test prefers not to propagate an error). `Event.wait`/`waitUncancelable` block the calling thread until `set()` has been called, with the documented acquire/release semantics described above making the writes in `captureRequestHeaders` visible to the waiting thread once `wait` returns. Optionally use `server.headers_captured.waitTimeout(std.testing.io, <bound>)` with a generous bound (e.g. a few seconds) and fail the test explicitly with a descriptive message if it returns `error.Timeout`, so a genuine server-side hang produces a clear test failure instead of an indefinite hang — this is a robustness improvement, not required to fix the race itself, and may be left as plain `wait`/`waitUncancelable` if BACKEND-DEV prefers to keep the change minimal; either is acceptable, the choice is not a correctness-affecting design decision.
...
5. **Ordering summary (the new, non-racy sequence):**
   - Test thread: `server.start()` → wait on `listening` event (via `std.testing.io`) → read `bound_port`, build `url_template` → call `st.executeHttpRequest(...)` (which opens the connection, sends the request, and blocks until the response is received) → wait on `headers_captured` event (via `std.testing.io`; in practice this returns immediately since `executeHttpRequest` already returned, meaning the HTTP round trip completed, meaning `captureRequestHeaders` must already have run and the event must already be set — but the explicit wait makes this a guarantee rather than an assumption) → read `first_trace_id` etc. → assert.
   - Server thread: bind → write `bound_port` → set `listening` (via `std.testing.io`) → accept → read headers into `first_*` fields via `captureRequestHeaders` → set `headers_captured` (via `std.testing.io`) → respond → loop condition false (`request_count == max_requests`) → `run()` returns → thread exits → `join()` (deferred) returns promptly.

### Net effect

The test thread never reads `first_trace_id` / `first_trace_id_len` / `first_idempotency_key` / `first_idempotency_key_len` / `first_custom_header` / `first_custom_header_len` until the background thread has explicitly signalled, via a proper synchronisation primitive with documented acquire/release semantics, that those exact writes have completed. This removes the data race by construction rather than relying on the incidental ordering a successful HTTP round-trip usually provides.
...
2. **The test thread MUST NOT read any `CaptureServer` field written by the background thread (`bound_port`, `first_trace_id`, `first_trace_id_len`, `first_idempotency_key`, `first_idempotency_key_len`, `first_custom_header`, `first_custom_header_len`, or their `second_*` counterparts if a future test reads them) without first waiting (via `std.testing.io`, `Event.wait`/`waitUncancelable`/`waitTimeout`) on a synchronisation primitive that the background thread signals strictly after writing that field.** A `defer`-scheduled `join()` that runs after the reads it was meant to guard does not satisfy this invariant — the wait must be sequenced in the code strictly before the read, not merely guaranteed to eventually run before the test function returns.
3. **Each readiness milestone gets its own signal.** "Bound and listening" and "first request's headers captured" are distinct events in the server's lifecycle and must not share one `Event`, because the test needs to act on each independently (read `bound_port` after the first; read `first_*` fields after the second).
4. **`server.join()` remains solely a thread-lifecycle cleanup step** after this change, not a data-visibility mechanism. Any future reviewer must not reintroduce reliance on `defer server.join()` ordering as a substitute for an explicit wait on the relevant `Event`.
...
6. **Every `Event.wait`/`waitUncancelable`/`waitTimeout`/`set` call must pass `std.testing.io` explicitly.** Unlike `std.Thread.ResetEvent`, `std.Io.Event`'s wait/set operations are not bare zero-argument methods — omitting the `Io` argument is a compile error, not a latent bug, but the design calls this out because it is the detail most likely to be miscopied from `ResetEvent`-shaped muscle memory.
...
- New field `bound_port: u16` (default `0`) — populated by the background thread inside `run()`, immediately after `listen()` succeeds, with the actual port the OS assigned (read from `listener.socket.address`). Read by the test thread only after waiting on `listening`.
- New field `listening: std.Io.Event` (default `.unset`) — set (via `.set(std.testing.io)`) by the background thread once `bound_port` has been written and the listener is ready to accept. Waited on (via `.wait(std.testing.io)`) by the test thread before reading `bound_port` / building `url_template`.
- New field `headers_captured: std.Io.Event` (default `.unset`) — set (via `.set(std.testing.io)`) by the background thread immediately after `captureRequestHeaders` returns for the first captured request, before `respond()` is called. Waited on (via `.wait(std.testing.io)`) by the test thread, after `executeHttpRequest` returns and before reading any `first_*` field.
- `run()` gains two new statements (prose, not code): one immediately after the `listen()` call succeeds, to write `bound_port` from `listener.socket.address` and then call `self.listening.set(std.testing.io)`; one immediately after the existing call to `captureRequestHeaders(self, &request)`, to call `self.headers_captured.set(std.testing.io)`.
- `TC-EXT-01-U08a`'s test body gains: construction with `.port = 0` instead of `.port = 18181`; a wait on `server.listening.wait(std.testing.io)` (or `try` it, matching `Event.wait`'s `Cancelable!void` return) after `server.start()` and before building `url_template`; dynamic construction of `url_template` (heap-allocated via the test's allocator, freed via `defer`) using `server.bound_port` instead of the `"http://127.0.0.1:18181/execute"` literal; a wait on `server.headers_captured.wait(std.testing.io)` after `st.executeHttpRequest(...)` returns and before the first `expectEqualStrings` call on a captured field.
...
- `Event.wait(io)` returns `Cancelable!void` — it can return a cancelation error in addition to blocking; the test should `try` it (propagating a cancelation as a test failure, which is the correct behaviour for this test binary) or use `waitUncancelable(io)` if cancelation is not a concern in this context. Either blocks indefinitely if the signal is never set (e.g. server thread crashed before signalling). BACKEND-DEV may choose `waitTimeout(io, timeout)` with a bounded timeout and an explicit `std.testing.expect(false)`-style failure with a descriptive message on `error.Timeout`, to avoid an indefinite hang masking a real defect as a stuck test run. This is a recommended robustness addition, not a hard requirement of this design.
- `listen()` failure (already handled today via `catch return` inside `run()`) — unchanged by this design; still out of scope. If BACKEND-DEV wants to surface a bind failure more loudly (e.g. so an ephemeral-port bind failure, which should be exceedingly rare, doesn't silently hang the test on the `listening` wait instead of the previous silent `run()` return), they may additionally have the `catch` branch call `self.listening.set(std.testing.io)` before returning so the test thread's wait does not hang forever on a bind failure — again a robustness improvement, not required to satisfy the two acceptance criteria in ISS-0207.

---

## Dependencies

- `std.Io.Event` — new dependency for this test file; standard library (`lib/std/Io.zig` in the vendored zig 0.16.0 toolchain), no vendoring required. Driven via `std.testing.io`, which is already available to both threads in this test binary.
- `std.Io.net.Server.socket.address` — already indirectly reachable via `listen_address.listen(io, ...)` in `run()`, which returns the `Server` value this design reads from; this design only adds a read of the already-returned value's `socket.address` field (an `IpAddress` containing the OS-resolved ephemeral port). No new accessor or type is introduced.
...
None. Both acceptance criteria (ephemeral port + happens-before synchronisation) are fully specified above; BACKEND-DEV has a concrete field list, a concrete statement-placement description for both the "set" and "wait" sides of each of the two `std.Io.Event`s (including the required `std.testing.io` argument on every call), and an explicit statement of which existing behaviour (`defer server.join()`) is retained and why it is no longer load-bearing for correctness.
