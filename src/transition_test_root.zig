...
//!
//! ISS-0132: transition.zig carries 26 in-file tests plus the ISS-0132
//! `checkAllAllocationFailures` harness, and none of them ever executed.
//! build.zig referenced the file only as an imported module (`transition_mod`),
//! never as the root of an `addTest`, so `zig build test` compiled it and ran
//! none of its test blocks — the same defect class as ISS-0102 (integration
//! test files wired into no build target), here in the engine's purest and
