...
// definition before the plan is computed so a failing gate creates no plan and
// no promotion_reviews row. tenant_context routes the gate's pool connection
// to the source tenant's schema.
...
pub fn handleSubmitPromotion(
    ...
    // definition BEFORE the plan is computed. A failing gate returns HTTP 422
    // and guarantees that no promotion plan is computed and no
