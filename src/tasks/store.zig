    ...
    pub fn listCursor(
        ...
        if (params.assignee_id) |aid| {
            ...
            if (params.include_group_membership_for_user) {
                const cond = std.fmt.allocPrint(
                    a,
                    "((assignee_type = 'USER' AND assignee_ref = ${d}) OR (assignee_type = 'GROUP' AND EXISTS (SELECT 1 FROM group_members gm WHERE gm.user_id = ${d}::uuid AND gm.group_id = tasks.assignee_ref)))",
                    .{ aid_idx, aid_idx },
                ) catch return TaskError.InvalidInput;
                conditions.append(a, cond) catch return TaskError.InvalidInput;
            } else {
