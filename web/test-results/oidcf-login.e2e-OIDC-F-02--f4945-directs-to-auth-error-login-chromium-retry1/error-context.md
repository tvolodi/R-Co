...
- Name: oidcf-login.e2e.spec.ts >> OIDC-F-02 — Callback error handling >> TC-OIDCF-04: navigating to /auth/callback with error params redirects to auth-error login
- Location: tests\e2e\oidcf-login.e2e.spec.ts:168:3

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://127.0.0.1:4173/auth/callback?error=access_denied&error_description=User+denied+access
Call log:
  - navigating to "http://127.0.0.1:4173/auth/callback?error=access_denied&error_description=User+denied+access", waiting until "load"
...
> 172 |     await page.goto('/auth/callback?error=access_denied&error_description=User+denied+access')
  ...
  211 |     ])
  212 | 
  213 |     // Assert the captured request targets Keycloak end-session endpoint
  214 |     expect(logoutRequest.url()).toContain(`${KEYCLOAK_REALM_URL}/protocol/openid-connect/logout`)
  215 | 
  216 |     await shot(page, 'TC05-02-after-logout-click')
  217 |     // VERDICT: Clicking logout after OIDC login navigates to Keycloak end-session endpoint
  218 |   })
  219 | })
  220 | 
```