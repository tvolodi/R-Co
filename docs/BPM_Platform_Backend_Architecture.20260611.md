...
**Version:** 1.1 · 2026-06-11 (correctness & consistency pass)  
**Based on:** BPM_Platform_Functional_Requirements.md and the implemented codebase through Stage 14  
**Supersedes:** 1.0 (2026-06-11); 0.1-draft (2026-05-20), which described only Stages 1–6.

> **Changelog (1.0 → 1.1).** This revision resolves internal contradictions and
> under-specifications surfaced by a correctness/consistency audit. **Schema:** `timers`
> now allows `FAILED`; `tasks` gains a `claimed_by` column distinct from the group/role
> assignee; `audit_log.resource_id` is now `TEXT`; `instances` records both
> `definition_snapshot` and the `artifact_hash` it executed against; the `token_hash`
> comment no longer implies bcrypt. **Spec:** the pure transition function returns
> `{state, emitted_events}`; webhook delivery uses a transactional outbox (true
> at-least-once); audit rows are written **inside** the state-change transaction; tokens are
> an identified multiset with explicit parallel-join counters; the scheduler uses one
> coordination model (`FOR UPDATE SKIP LOCKED` shared queue) and the startup sweep takes the
> same lock; the rate limiter is a shared-store sliding window (global in HA); OIDC tokens
> have defined cache/rate-limit keying; task completion is guarded against terminal instances;
> variable merges validate two-phase (all-or-nothing); internal events derive deterministic
> idempotency keys. **Docs:** §5 is annotated for tenant placement and the SPT coexistence
> read/write authority is specified.
...
Event Store           ┐
  ...
  │
  ▼
Webhook Dispatcher    ── outbox: delivery rows written IN the transaction above; workers fire
                          instance.started, task.activated async after commit
  │
  ▼
...
Event Store ATOMIC TRANSACTION
  ...
  │
  ▼
Webhook Dispatcher ── outbox rows committed in the transaction above; async outbound calls after commit
  │
  ▼
...
SELECT timers WHERE fire_at <= now() FOR UPDATE SKIP LOCKED
  │  (SKIP LOCKED lets ANY node safely take un-locked due timers; no two nodes
  │   ever grab the same row, so multiple nodes share the load without double-firing)
  │
  ▼
For each due timer:
  ATOMIC TRANSACTION
...
All migrations are versioned, idempotent SQL files applied in sequence.

**Tenant placement (read this before the DDL).** Under schema-per-tenant ([§11](#11-multi-tenancy--schema-per-tenant-isolation)),
the tables below are **per-tenant**: they are created inside each `tenant_<slug>` schema by the
per-tenant migration set, so every tenant has its own `events`, `definitions`, `instances`,
`tasks`, `timers`, `audit_log`, `dead_letter_items`, and `webhook_subscriptions`. The
**platform-global** tables — the tenant registry (`public.tenants`), realm/tenant bindings,
and migration tracking — live in `public` and are created by `GBL-`prefixed migrations. The
DDL in §5.1 is shown without a `tenant_id` column on purpose: isolation is structural (the
connection's `search_path` exposes exactly one tenant schema), not predicate-based. The
legacy ADP model added a `tenant_id` column + RLS to these same tables; that path is being
retired by the SPT migration ([§11.3](#113-schema-per-tenant-migration-spt--in-progress)).

**Migration state** is tracked per schema: `public.schema_migrations` records `GBL-`
migrations, and each `tenant_<slug>.schema_migrations` records that tenant's per-tenant
migrations. The numbered migrations shown here (001–012) are the original kernel set; the
multi-tenant and identity migrations (027–037, 069–070, and the `GBL-*` series) are catalogued
in §9 and §11–§13, not repeated here.
...
CREATE TABLE events (
    ...
    sequence_num    BIGINT GENERATED ALWAYS AS IDENTITY,  -- total ordering within the tenant's event stream
    instance_id     UUID NOT NULL,
    event_type      TEXT NOT NULL,
    payload         JSONB NOT NULL DEFAULT '{}',
    metadata        JSONB NOT NULL DEFAULT '{}',          -- trace IDs, source tags
    actor_id        UUID,
    idempotency_key TEXT UNIQUE NOT NULL,                 -- unique within the tenant's events table (per-tenant scope under schema-per-tenant)
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_events_instance_seq ON events (instance_id, sequence_num);
CREATE INDEX idx_events_global_seq   ON events (sequence_num);            -- whole-tenant stream (projection rebuilds)
...
CREATE TABLE instances (
    ...
    definition_snapshot JSONB NOT NULL,   -- inline copy of graph at start time (PD-08); authoritative for execution
    artifact_hash     TEXT,               -- content hash of the promoted artifact this instance ran against (ADP-05); provenance/reproducibility pointer
...
CREATE TABLE tasks (
    ...
    assignee_ref  TEXT,                 -- the assignment POOL (user id, group, or role)
    claimed_by    UUID,                 -- individual who claimed a GROUP/ROLE task; NULL until claimed
    form_schema   JSONB,
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE timers (
    ...
    status        TEXT NOT NULL CHECK (status IN ('PENDING','FIRED','CANCELLED','FAILED')),
...
CREATE TABLE api_tokens (
    ...
    token_hash  TEXT UNIQUE NOT NULL,  -- SHA-256 of the high-entropy token (deterministic, supports lookup); plaintext never stored
...
CREATE TABLE audit_log (
    ...
    resource_id    TEXT NOT NULL,        -- TEXT, not UUID: some resources are text-keyed (role_name, event_type, definition name)
    before_state   JSONB,
    after_state    JSONB,
...
- Task worker queue: `tasks(assignee_type, assignee_ref) WHERE status='PENDING' AND claimed_by IS NULL` — unclaimed pool work
- Claimed work: `tasks(claimed_by) WHERE status='PENDING'` — "my tasks" lookups
...
[4] RBAC middleware           — check actor roles against resource + action permission matrix
                                 → 403 if insufficient
[5] Rate limiter              — sliding-window counter in a SHARED store (Postgres or Redis,
                                 BPM_RATE_LIMIT_BACKEND), keyed by (tenant_id, principal) so the
                                 per-token RPM limit (API-10) is enforced GLOBALLY across all
                                 nodes, not per-node. `principal` is the api_token id for local
                                 tokens and the (realm, sub) pair for OIDC tokens.
...
[9] Audit commit (in-txn)     — for state-changing verbs the handler writes the audit_log row
                                 INSIDE its own state-change transaction (not as a post-handler
                                 step), so audit commits atomically with the change; see §6.8, §7.1
...
**Definition snapshot (PD-08):** At instance start, the full `graph` JSON is copied into
`instances.definition_snapshot`, which is **authoritative for execution** — the definition
registry is never queried again for that instance's run. Separately,
`instances.artifact_hash` records the content hash of the promoted artifact the instance ran
against (§15.1); it is a provenance/reproducibility pointer, not a second execution source, so
...
pub const TransitionResult = struct {
    state:           InstanceState,   // the new derived state
    emitted_events:  []Event,         // events the orchestrator must append in the same txn
};
...
- The function returns **both** the new state and the list of events it produced
  (`TASK_ACTIVATED`, `VARIABLE_OVERWRITTEN`, `INSTANCE_COMPLETED`, `EXECUTION_ERROR`, …).
  Returning the events explicitly is required: events such as `VARIABLE_OVERWRITTEN`
  (old + new value) and `EXECUTION_ERROR` cannot be reconstructed from the final state by
  diffing, and the orchestrator must persist exactly the events the engine decided on.
- No database calls, no network calls, no logging inside this function. The triggering event
  is **not** re-appended by the engine; the orchestrator appends the trigger plus
  `emitted_events` atomically.
...
pub const Token = struct {
    token_id: Uuid,    // stable identity; survives replay
    node_id:  NodeId,  // current position
};

pub const InstanceState = struct {
    instance_id:   Uuid,
    status:        InstanceStatus,     // ACTIVE | COMPLETED | CANCELLED | ERROR
    active_tokens: []Token,            // identified multiset: two tokens may sit on one node
    join_counters: std.AutoHashMap(NodeId, JoinCounter), // per parallel-join arrival state
    variables:     std.json.ObjectMap, // mutable variable map
    last_seq:      u64,
};
```

Tokens are an **identified multiset**, not a set of node IDs: loops and reconverging parallel
branches can legitimately place more than one token on the same node, and each token needs a
stable `token_id` so replay reproduces the exact arrangement. `join_counters` is part of the
state (and therefore part of what replay rebuilds) so PARALLEL_GATEWAY joins are
deterministic. The `instances.active_tokens` JSONB column stores the `[{token_id, node_id}]`
array; a sibling `instances.join_counters` JSONB column persists the join state.
...
- **PARALLEL_GATEWAY (join):** On each arriving token, increment the join's entry in
  `state.join_counters` and consume that token. Advance (emit one outgoing token) only when the
  counter equals the number of branches still active for this join — branches cancelled before
  reaching the join are subtracted from the expected count. Because the counter lives in
  `InstanceState`, the join resolves identically under live execution and event replay.

**Variable collision policy (EE-09):**

The merge is **two-phase and all-or-nothing**: every key is validated before any value is
written, so a single bad key never leaves the instance with a half-applied merge.

```
# Phase 1 — validate; no state change, no events emitted yet
for each (key, new_value) in task_output:
    if schema_defined(key) and not schema_valid(key, new_value):
        emit EXECUTION_ERROR event (key, reason); return   # nothing overwritten

# Phase 2 — all keys valid: apply atomically
for each (key, new_value) in task_output:
...
If validation fails the only event emitted is `EXECUTION_ERROR`; the variable map is untouched,
so a retry sees the same pre-merge state.
...
**Error recovery (EE-10):** Instances in ERROR status are frozen. The dead letter API (OBS-05)
allows operators to supply a `retry` command or a `discard` command (which terminates the
instance as CANCELLED). Because a deterministic engine re-presented with the *identical*
triggering event reproduces the *identical* error, `retry` supports two convergent forms:
`retry` after the operator has corrected the underlying cause (e.g. activated a fixed
definition version or amended instance variables via an authorised correction event), or
`retry-with-input` which re-presents the triggering event with operator-supplied corrected
payload. A bare retry against unchanged definition+data is rejected with a hint that nothing
has changed, rather than silently looping.
...
| `complete(task_id, output_vars, actor_id)` | Validate actor has rights; **reject (HTTP 409) if the parent instance is not ACTIVE** (ERROR/CANCELLED/COMPLETED instances accept no task completion); call Execution Engine (pure); persist result atomically |
...
**Claim semantics for GROUP/ROLE assignments:** `assignee_ref` holds the *pool* (the group or
role) and is set at activation; the individual who picks up the task is recorded in the
separate `claimed_by` column, which is NULL until claimed. A task is claimed by the first
worker to call `POST /tasks/:id/claim`; double-claim is prevented by an optimistic
compare-and-set:

```sql
UPDATE tasks SET claimed_by = $worker
WHERE task_id = $id AND claimed_by IS NULL AND status = 'PENDING';
-- 0 rows updated → already claimed (HTTP 409)
```
...
startup
  ...
  └─► poll loop:
        ...
        │  (No whole-cycle leader lock. Coordination is row-level via SKIP LOCKED so
        │   every node can drain the due-timer queue concurrently — this removes the
        │   single-node bottleneck of a cluster-wide scheduler lock.)
        │
...
**Missed timer recovery (SCH-05):** On startup (before entering the poll loop), the scheduler
runs one sweep for all `fire_at <= now()` timers, fired with `is_late_fire = TRUE`. To stop
several nodes restarting together (e.g. a rolling deploy) from sweeping the same timers, the
startup sweep runs under `pg_try_advisory_lock(SCHEDULER_STARTUP_LOCK_ID)`: only the node that
acquires it performs the sweep; the others proceed straight to the normal poll loop (where
SKIP LOCKED already prevents double-firing). The sweep itself still selects `FOR UPDATE SKIP
LOCKED` as defence in depth.
...
**Source of truth for roles.** `api_tokens.roles[]` is a deliberate **point-in-time grant
snapshot**: the roles a token carries are fixed at issuance and are authoritative for that
token until it expires or is revoked. `user_roles` is the authoritative registry for a *user*;
changing it does not retroactively widen or narrow already-issued tokens (issue a new token, or
revoke). This avoids the ambiguity of two stores disagreeing per request. For interactive OIDC
principals there is no `api_tokens` row at all — roles come from validated token claims via the
claim→role mapping ([§12.2](#122-realm-per-tenant-oidc-stage-srcoidc)), re-evaluated each request.

**OIDC access tokens (hot path).** OIDC tokens are validated against the tenant realm's
issuer/JWKS rather than by table lookup. They are cached keyed by `(realm, jti)` with a TTL
bounded by the token's own `exp`, and revoked via realm logout/`jti` denylist rather than the
`token_revoked` channel. Rate limiting keys them on `(tenant_id, realm, sub)` per step 5 above.
...
**Audit log (OBS-03):** One `audit_log` row is written per state-changing request, capturing
`before_state` and `after_state` as JSONB snapshots. The write happens **inside the handler's
state-change transaction**, so the audit row and the events/projection it describes commit (or
roll back) together — there is no window in which a change is committed without its audit
record, and no audit row survives for a change that rolled back. (Pipeline step 9 is the
logical "audit" phase; the physical INSERT is enlisted in the handler transaction opened at
step 8.) The audit trail is thus a transactionally consistent companion to the append-only
event log, which remains the tamper-evident ground truth.
...
**Responsibility:** At-least-once outbound HTTP delivery of platform events to registered
subscribers, guaranteed by a **transactional outbox** (not a best-effort in-process signal).

**Dispatch flow:**

1. **In the same transaction that appends a publishable event,** the handler also inserts one
   `webhook_deliveries` row (status `PENDING`) per matching active subscription. Because the
   delivery rows commit atomically with the event, "event committed" implies "delivery
   enqueued" — a crash immediately after commit cannot lose a webhook. This is what makes
   delivery at-least-once.
2. After commit, the handler fires an **in-process notification** to wake the worker pool
   immediately. This is purely a latency optimisation; if the process dies before the workers
   run, the persisted `PENDING` rows are picked up by a periodic outbox sweep (and on startup),
   so delivery still happens. Workers also fall back to polling `webhook_deliveries` so no
   notification is strictly required.
3. A delivery worker pool (configurable size, default 4 workers) claims `PENDING`/due rows with
   `FOR UPDATE SKIP LOCKED` and issues HTTP POST calls with:
...
**Delivery state** is persisted in a per-tenant `webhook_deliveries` table (added in Stage 6;
columns: `delivery_id`, `subscription_id`, `event_id`, `status` ∈
`PENDING|DELIVERED|FAILED|RETRYING`, `attempt`, `next_attempt_at`, `last_error`,
`created_at`). It is both the outbox (guaranteeing delivery survives restarts) and the
retry/back-off ledger.
...
**Keys for client commands vs. engine-emitted events.** Client-initiated commands carry a
caller-supplied `Idempotency-Key`. Events the engine emits as a cascade (`TASK_ACTIVATED`,
`TIMER_FIRED`, `VARIABLE_OVERWRITTEN`, `INSTANCE_COMPLETED`, …) derive a **deterministic** key
so that a replayed or retried transition cannot create duplicates and cannot spuriously
collide with unrelated events. The derivation is a stable hash of
`(instance_id, triggering_event_seq, node_id, emitted_event_type, ordinal_within_transition)`.
Determinism means re-running the same transition over the same trigger yields the same keys,
...
For instances with up to 10,000 events, this must complete in ≤ 5 seconds (NFR-04). The pure
transition function has no I/O, so the bottleneck is the `SELECT`. The
`(instance_id, sequence_num)` index returns the rows already ordered by `sequence_num` for the
instance (an ordered index scan; heap fetches are effectively localised because events for an
instance are appended close together in time). For instances whose events carry overflow
payloads, replay also joins `event_payloads_overflow` to reconstruct full payloads. At 10,000
rows this is comfortably within the 5-second target; very large instances can additionally be
served from periodic state snapshots rather than full replay.
...
| `BPM_RATE_LIMIT_RPM` | `1000` | Requests per minute per principal (global across nodes) |
| `BPM_RATE_LIMIT_BACKEND` | `postgres` | Shared counter store: `postgres` or `redis` |
...
One process handles HTTP, scheduler, and webhook dispatch. The scheduler drains due timers
with `FOR UPDATE SKIP LOCKED`; the same code path works unchanged from one node to many (no
special-casing).
...
All nodes share the same PostgreSQL instance. Timer firing scales across nodes: each node
selects due timers `FOR UPDATE SKIP LOCKED`, so the work is shared and no row is fired twice —
there is no cluster-wide scheduler lock to serialise on. (Only the one-shot startup
missed-timer sweep takes a short advisory lock; see §6.6.) HTTP requests can be served by any
node because all state lives in PostgreSQL.
...
> The items below are **open implementation/design questions**. A separate class of *internal
> contradictions* (schema-vs-spec mismatches, the transition-function return type, webhook
> delivery guarantee, audit transactionality, scheduler locking, token modelling, rate-limit
> scope, OIDC keying, etc.) was resolved in the v1.1 correctness pass — see the changelog at
> the top of this document.

| # | Question / Risk | Status |
|---|---|---|
| 1 | **CEL interpreter availability for Zig** | **Partially resolved.** Gateway conditions currently evaluate on the vendored CEL interpreter (`vendor/cel`) — `src/engine/transition.zig` imports and calls `cel.evaluate`. A pure-Zig expression engine (`src/expr/`) has been built and unit-tested as the intended replacement, but is **not yet wired into the engine path** (no production module imports `expr`). Cutover from `vendor/cel` to `src/expr/` remains open work. **Added risk:** stored gateway conditions are written in CEL, so cutover must either guarantee `src/expr/` evaluates them with identical semantics or re-validate/translate every stored definition — a differential test suite (CEL vs. expr over a corpus of stored conditions) is the gating control before cutover. |
...
**Authority during coexistence (SPT-02..04 pending).** To avoid split-brain between the legacy
`public` tables (tenant_id + RLS) and the new `tenant_<slug>` schemas, exactly one path is
authoritative **per tenant at a time**, flagged by `public.tenants.storage_mode ∈
{LEGACY_RLS, SCHEMA}`:

- A tenant still in `LEGACY_RLS` reads and writes only the `public` tables under its `tenant_id`
  + RLS predicate; its tenant schema (if pre-provisioned) receives no live writes.
- Migration flips the tenant to `SCHEMA` **only after** its rows are copied in and verified,
  inside the cutover transaction. From that point all reads and writes for the tenant go to its
  schema, and the RLS predicate is left in place purely as a belt-and-braces guard until SPT-03
  removes it globally.

There is never a period in which the same tenant is served writes on one path and reads on the
other; the `storage_mode` flag is resolved once during tenant-context resolution (§11.2) and
pins the connection's `search_path` accordingly.
...
reference the artifact hash they executed against in `instances.artifact_hash` (ADP-05),
making executions reproducible; the inline `definition_snapshot` (PD-08) remains the
authoritative execution source, while the hash ties the run to a promotable, content-addressed
artifact.
