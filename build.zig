...
pub fn build(b: *std.Build) void {
    ...
    const iss104_integration_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("tests/integration/iss104_artifact_hash_test.zig"),
            .target = target,
            .optimize = optimize,
            .imports = integration_imports,
        }),
    });
    const run_iss104_integration_tests = b.addRunArtifact(iss104_integration_tests);
    run_iss104_integration_tests.setCwd(b.path("."));
    run_iss104_integration_tests.setEnvironmentVariable("BPM_MIGRATIONS_DIR", migrations_dir);
    ...
    test_integration_step.dependOn(&run_iss104_integration_tests.step);
    ...
    const test_integration_iss104_step = b.step("test-integration-iss104", "Run ISS-104 artifact_hash column integration tests (requires BPM_TEST_DB_URL)");
    test_integration_iss104_step.dependOn(&clean_test_db.step);
    test_integration_iss104_step.dependOn(&run_iss104_integration_tests.step);
